...
export async function sendEmail({ to, subject, html, text }) {
  if (!isEmailConfigured()) {
    ...
    console.warn('[EMAIL] SMTP not configured. Add SMTP_HOST, SMTP_USER, SMTP_PASS to Render Environment Variables.');
    return { simulated: true };
  }
  try {
    const port     = parseInt(process.env.SMTP_PORT || '465', 10);
    const isSecure = process.env.SMTP_SECURE !== 'false'; // default: true (SSL port 465)
