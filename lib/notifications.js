...
// ── HTTP API Provider 3: Resend — Uses HTTPS Port 443 (No IP Whitelisting required!) ──
async function sendViaResend({ to, subject, html, text }) {
  ...
  try {
    ...
    const data = await res.json();
    if (res.ok) {
      console.log(`[RESEND HTTPS SUCCESS] Delivered to ${to} | Id: ${data.id}`);
      return { success: true, messageId: data.id, provider: 'resend' };
    } else {
      console.error(`[RESEND HTTPS ERROR]`, data);
      return { success: false, error: data.message || 'Resend API error' };
    }
  } catch (err) {
    console.error(`[RESEND HTTPS EXCEPTION]`, err.message);
    return { success: false, error: err.message };
  }
}
...
export async function sendEmail({ to, subject, html, text }) {
  ...
  // Method 3: Try Resend HTTPS API (Port 443 — No IP restrictions)
...
export function buildOrderMessages(order) {
  ...
  const appBaseUrl = process.env.PUBLIC_APP_URL || process.env.RENDER_EXTERNAL_URL || 'https://restaurant-pos-643r.onrender.com';
  const trackUrl = `${appBaseUrl}/?tab=track&orderId=${order.id}`;
