...
// Rewrite `?` positional params to Postgres `$1,$2,…`, sanitize string quotes, and convert multi-argument MAX(a, b) -> GREATEST(a, b)
function toPg(sql) {
  let i = 0;
  let pgSql = sql
    .replace(/=\s*"([^"]+)"/g, "= '$1'")
    .replace(/\bIN\s*\(\s*"([^"]+)"\s*\)/g, "IN ('$1')")
    .replace(/\bMAX\s*\(\s*([^,\)]+)\s*,/gi, 'GREATEST($1,');
