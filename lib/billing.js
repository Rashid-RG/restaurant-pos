...
export async function resolveAndCalculateBill(
  ...
  for (const item of items) {
    let dbItem = await dbGet('SELECT * FROM menu_items WHERE id = ? AND tenant_id = ?', [item.menuItemId || item.id, tenantId]);
    if (!dbItem) {
      dbItem = await dbGet('SELECT * FROM menu_items WHERE id = ?', [item.menuItemId || item.id]);
    }
    if (!dbItem) {
      throw new Error(`Menu item not found: ${item.name || item.menuItemId || item.id}`);
    }
    if (typeof dbItem.stock === 'number' && dbItem.stock < item.quantity) {
      throw new Error(`Insufficient stock for item: ${dbItem.name || dbItem.id}`);
    }
    ...
    if (item.selectedModifiers && Array.isArray(item.selectedModifiers)) {
      for (const modSelection of item.selectedModifiers) {
        const dbMod = await dbGet(
          'SELECT id, name, priceDelta FROM modifiers WHERE id = ? AND menuItemId = ? AND tenant_id = ?',
          [modSelection.id, dbItem.id, tenantId]
        );
        if (dbMod) {
  ...
  if (promoCode) {
    const promo = await dbGet(
      'SELECT * FROM promotions WHERE code = ? AND isActive = 1 AND tenant_id = ?',
      [promoCode.toUpperCase(), tenantId]
    );
    if (promo) {
  ...
  const serviceChargeSetting = await dbGet('SELECT value FROM settings WHERE tenant_id = ? AND key = ?', [tenantId, 'serviceChargeRate']);
  ...
  const taxRateSetting = await dbGet('SELECT value FROM settings WHERE tenant_id = ? AND key = ?', [tenantId, 'taxRate']);
